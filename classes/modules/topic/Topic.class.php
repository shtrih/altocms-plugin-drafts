...
class PluginDrafts_ModuleTopic extends PluginDrafts_Inherit_ModuleTopic
{
    ...
    public function GetCountDraftsPersonalByUser($sUserId) {
        ...
        $s       = serialize($aFilter);
        if (false === ($data = E::ModuleCache()->Get("topic_draft_count_user_{$s}"))) {
            $data = $this->oMapperTopic->GetCountTopics($aFilter);
            E::ModuleCache()->Set($data, "topic_draft_count_user_{$s}", array("topic_draft_update_user_{$sUserId}"), 60 * 60 * 24);
        }
